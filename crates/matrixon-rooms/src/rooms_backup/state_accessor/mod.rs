...
impl Service {
    ...
    pub fn get_join_rules(
        &self,
        room_id: &RoomId,
    ) -> Result<Option<RoomJoinRulesEventContent>> {
...
mod tests {
    ...
    use ruma::{
        ...
        room_id, server_name, user_id, EventId, RoomId,
    };
    use std::collections::HashMap;
    use std::sync::Arc;

    /// Test StateAccessor cache functionality
    #[tokio::test]
    async fn test_state_accessor_cache_functionality() {
        // Test cache initialization and basic operations
        let cache_size = NonZeroUsize::new(1000).unwrap();
        let server_cache: Mutex<LruCache<(OwnedServerName, u64), bool>> = 
            Mutex::new(LruCache::new(cache_size.into()));
        let user_cache: Mutex<LruCache<(OwnedUserId, u64), bool>> = 
            Mutex::new(LruCache::new(cache_size.into()));

        // Test cache operations
        assert_eq!(server_cache.lock().unwrap().len(), 0);
        assert_eq!(user_cache.lock().unwrap().len(), 0);
    }
