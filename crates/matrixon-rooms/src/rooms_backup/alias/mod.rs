...
impl Service {
    ...
    fn user_can_remove_alias(&self, alias: &RoomAliasId, user_id: &UserId) -> Result<bool> {
        let Some(room_id) = self.resolve_local_alias(alias)? else {
            return Err(Error::BadRequest(crate::MatrixErrorKind::NotFound, "Alias not found."));
        };
...
mod tests {
    ...
    fn test_error_handling_patterns() {
        ...
        for (error_type, description) in error_scenarios {
            ...
            match error_type {
                "NotFound" => {
                    let error = Error::BadRequest(crate::MatrixErrorKind::NotFound, description);
                    assert!(format!("{}", error).contains(description), "Should contain description");
                }
                "Forbidden" => {
                    let error = Error::BadRequest(crate::MatrixErrorKind::Forbidden, description);
                    assert!(format!("{}", error).contains(description), "Should contain description");
                }
                _ => {
