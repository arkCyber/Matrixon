...
impl PduCount {
    ...
    pub fn try_from_string(token: &str) -> Result<Self> {
        ...
        .map_err(|_| Error::BadRequest(crate::MatrixErrorKind::InvalidParam, "Invalid pagination token."))
    }
    ...
    pub async fn append_pdu<'a>(
        ...
        for appservice in services().appservice.read().await.values() {
            ...
            if pdu.kind == TimelineEventType::RoomMember {
                ...
                {
                    let appservice_uid = appservice.registration.sender()_localpart.as_str();
