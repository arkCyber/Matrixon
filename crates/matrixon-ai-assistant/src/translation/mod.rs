...
impl TranslationService {
    ...
    pub async fn translate(&self, request: &TranslationRequest) -> Result<TranslationResponse> {
        ...
        if !self.config.enabled {
            return Err(Error::BadRequest("Translation service is not enabled".to_string()));
        }

        // Validate target language
        if !self.is_language_supported(&request.target_language).await {
            return Err(Error::BadRequest("Unsupported target language".to_string()));
        }
        ...
        // Detect source language if empty or auto-detect enabled
        let source_language = if request.source_language.is_empty() && self.config.auto_detect_language {
            self.detect_language(&request.text).await?.language
        } else if request.source_language.is_empty() {
        ...
        let translation_result = self.perform_translation(request, &source_language).await?.to_owned();
    ...
    #[instrument(level = "debug", skip(self))]
    pub async fn is_language_supported(&self, lang_code: &str) -> bool {
    ...
    /// Get language name
    #[instrument(level = "debug", skip(self))]
    pub async fn get_language_name(&self, lang_code: &str) -> String {
        let languages = self.supported_languages.read().await;
    ...
    async fn translate_with_model(&self, text: &str, source: &str, target: &str) -> Result<String> {
        let prompt = format!(
            "Translate the following text from {} to {}:\n\n{}",
            source, target, text
        );

        let response = self.llm_integration.generate_response(&LlmRequest {
            model: self.config.translation_provider.clone(),
...
mod tests {
    use super::*;
    use std::sync::Arc;
    use crate::llm_integration::{LlmIntegration, LlmProviderConfig, LlmProvider};
    ...
    #[tokio::test]
    async fn test_translation_service() {
        let config = TranslationConfig {
            enabled: true,
            default_source_language: "en".to_string(),
            default_target_language: "en".to_string(),
        ...
        let llm_integration = Arc::new(LlmIntegration::new(providers).await.unwrap());
        let service = TranslationService::new(config, llm_integration).await.unwrap();
