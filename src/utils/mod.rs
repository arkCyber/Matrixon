...
/// Parses bytes into a string with UTF-8 validation
/// 
/// Converts raw bytes to a valid UTF-8 string, ensuring data integrity
/// for Matrix protocol text content.
/// 
/// # Performance Target
/// <5μs execution time for typical Matrix content
/// 
/// # Arguments
/// * `bytes` - Raw byte slice to convert
/// 
/// # Returns
/// * `std::result::Result<String, std::string::FromUtf8Error>` - Parsed string or error
/// 
/// # Errors
/// * Returns error if bytes contain invalid UTF-8 sequences
/// 
/// # Examples
/// ```
...
/// let string = string_from_bytes(bytes).unwrap();
...
/// Converts Matrix content to canonical JSON object format
/// 
/// Simple wrapper for canonical JSON conversion without complex error handling
/// 
/// # Arguments
/// * `value` - Any serializable value to convert
/// 
/// # Returns
/// * `std::result::Result<CanonicalJsonObject, CanonicalJsonError>` - Canonical JSON object or error
pub fn to_canonical_object<T: serde::Serialize>(
    value: T,
) -> std::result::Result<CanonicalJsonObject, CanonicalJsonError> {
    ...
    match json_value {
        ...
        _ => {
            use serde::ser::Error;
            Err(CanonicalJsonError::SerDe(serde_json::Error::custom(
...
/// Deserialize from string with custom error handling
/// 
/// # Arguments
/// * `deserializer` - Serde deserializer
/// 
/// # Returns
/// * `std::result::Result<T, D::Error>` - Deserialized value or deserializer error
