    ...
    pub async fn answer_question(&self, request: &QaRequest) -> Result<QaResponse> {
        ...
        if !self.config.enabled {
            return Err(Error::BadRequestString(
                ...
                "Q&A bot is not enabled".to_string(),
            ));
        }
...
mod tests {
    ...
    async fn create_test_qa_bot() -> QaBot {
        ...
        QaBot::new(config, llm_integration).await.unwrap()
    }

    #[tokio::test]
    async fn test_qa_bot_creation() {
        let qa_bot = create_test_qa_bot().await;
        assert!(qa_bot.config.enabled);
        assert_eq!(qa_bot.config.confidence_threshold, 0.7);
    }

    #[tokio::test]
    async fn test_question_categorization() {
        let qa_bot = create_test_qa_bot().await;
        
        assert_eq!(qa_bot.categorize_question("How do I install matrixon?").await, "Installation & Setup");
        assert_eq!(qa_bot.categorize_question("What is Matrix protocol?").await, "Matrix Protocol");
        assert_eq!(qa_bot.categorize_question("How do I create a room?").await, "Room Management");
        assert_eq!(qa_bot.categorize_question("I have an error").await, "Troubleshooting");
        assert_eq!(qa_bot.categorize_question("Random question").await, "General");
    }
    ...
    async fn test_knowledge_base_search() {
        ...
        let results = qa_bot.search_knowledge_base("What is Matrix?", Some("en")).await.unwrap();
        assert!(!results.is_empty());
        assert!(results.iter().any(|entry| entry.title.contains("Matrix")));
    }
    ...
    async fn test_cache_key_generation() {
        ...
        let request = QaRequest {
            question: "What is matrixon?".to_string(),
        ...
        let cache_key = qa_bot.generate_cache_key(&request);
        assert!(cache_key.starts_with("qa_"));
    }

    #[tokio::test]
    async fn test_knowledge_entry_management() {
        let qa_bot = create_test_qa_bot().await;
        ...
        qa_bot.add_knowledge_entry(entry).await.unwrap();
        
        let stats = qa_bot.get_statistics().await;
        assert!(stats.knowledge_base_size > 5); // Default entries + test entry

        qa_bot.remove_knowledge_entry("test_entry").await.unwrap();
    }
} 
