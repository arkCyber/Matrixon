...
use super::{RecommendationConfig, RecommendationAlgorithm};
...
mod tests {
    ...
    async fn create_test_recommendation_engine() -> RecommendationEngine {
        ...
        RecommendationEngine::new(config).await.unwrap()
    }

    #[tokio::test]
    async fn test_recommendation_engine_creation() {
        let engine = create_test_recommendation_engine().await;
        assert!(engine.config.enabled);
        assert_eq!(engine.config.max_recommendations, 10);
    }

    #[tokio::test]
    async fn test_user_similarity_calculation() {
        let engine = create_test_recommendation_engine().await;
        
        let user1_interactions = HashMap::from([
            ("item1".to_string(), 0.8),
            ("item2".to_string(), 0.6),
        ]);
        
        let user2_interactions = HashMap::from([
            ("item1".to_string(), 0.9),
            ("item2".to_string(), 0.7),
            ("item3".to_string(), 0.5),
        ]);

        let similarity = engine.calculate_user_similarity(&user1_interactions, &user2_interactions);
    ...
    async fn test_content_similarity_calculation() {
        ...
        let user_interests = HashMap::from([
            ("tech".to_string(), 0.9),
            ("programming".to_string(), 0.8),
        ]);
        
        let item_features = HashMap::from([
            ("tech".to_string(), 0.7),
            ("programming".to_string(), 0.9),
            ("design".to_string(), 0.3),
        ]);

        let similarity = engine.calculate_content_similarity(&user_interests, &item_features);
        assert!(similarity > 0.0);
        assert!(similarity <= 1.0);
    }

    #[tokio::test]
    async fn test_fallback_recommendations() {
        let engine = create_test_recommendation_engine().await;
        ...
        let recommendations = engine.fallback_recommendations(&request).await.unwrap();
        assert!(!recommendations.is_empty());
        assert!(recommendations.iter().all(|r| r.recommendation_type == "rooms"));
    }
    ...
    async fn test_cache_key_generation() {
        ...
        assert!(cache_key.starts_with("rec_@test:example.com_"));
    }

    #[tokio::test]
    async fn test_user_interaction_addition() {
        let engine = create_test_recommendation_engine().await;
        
        let interaction = UserInteraction {
            user_id: "@test:example.com".to_string(),
            item_id: "#testroom:example.com".to_string(),
            item_type: "room".to_string(),
            interaction_type: "join".to_string(),
            timestamp: 1703001600000,
            score: 0.8,
            context: HashMap::new(),
        };

        engine.add_user_interaction(interaction).await.unwrap();
        
        let interactions = engine.get_user_interactions("@test:example.com").await;
        assert!(interactions.contains_key("#testroom:example.com"));
        assert_eq!(interactions["#testroom:example.com"], 0.8);
    }
} 
