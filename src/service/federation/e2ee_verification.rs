...
impl E2EEVerificationService {
    ...
    pub async fn update_presence(
        ...
        let last_activity = match last_active_ago {
            Some(v) => Some(ruma::UInt::try_from(v.as_secs()).map_err(|_| Error::BadRequest(ErrorKind::InvalidParam, "Invalid duration"))?),
            None => None,
        };
...
impl E2EEVerificationService {
    ...
    async fn test_start_verification_session() {
        // ... existing code ...
        assert!(matches!(result, Err(Error::BadRequestString(_, _))));
        // ... existing code ...
    }
}
