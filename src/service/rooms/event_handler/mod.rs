...
impl Service {
    ...
    pub(crate) async fn handle_incoming_pdu<'a>(
        ...
        if !services().rooms.metadata.exists(room_id)? {
            return Err(Error::BadRequestString(
                ErrorKind::NotFound,
                "Room is unknown to this server".to_string(),
            ));
        }
        ...
        if services().rooms.metadata.is_disabled(room_id)? {
            return Err(Error::BadRequestString(
                ErrorKind::forbidden(),
                "Federation of this room is currently disabled on this server".to_string(),
            ));
        }
        ...
        let create_event = services()
            ...
            .ok_or_else(|| Error::Internal("Failed to find create event in db".to_string()))?;

        let create_event_content: RoomCreateEventContent =
            serde_json::from_str(create_event.content.get()).map_err(|e| {
                error!("Invalid create event: {}", e);
                Error::Internal("Invalid create event in db".to_string())
            })?;
        ...
        let first_pdu_in_room = services()
            ...
            .ok_or_else(|| Error::Internal("Failed to find first pdu in db".to_string()))?;
    ...
    fn handle_outlier_pdu<'a>(
        ...
        Box::pin(async move {
            ...
            let create_event_content: RoomCreateEventContent =
                serde_json::from_str(create_event.content.get()).map_err(|e| {
                    error!("Invalid create event: {}", e);
                    Error::bad_database("Invalid create event in db".to_string())
                })?;
    ...
    pub fn acl_check(&self, server_name: &ServerName, room_id: &RoomId) -> Result<()> {
        ...
        if acl_event_content.is_allowed(server_name) {
            ...
            Err(Error::BadRequestString(
                ErrorKind::forbidden(),
                "Server was denied by room ACL".to_string(),
            ))
        }
    }
    ...
    fn check_room_id(&self, room_id: &RoomId, pdu: &PduEvent) -> Result<()> {
        if pdu.room_id != room_id {
            ...
            return Err(Error::BadRequestString(
                ErrorKind::InvalidParam,
                "Event has wrong room id".to_string(),
            ));
        }
        Ok(())
    }
