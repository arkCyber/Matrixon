...
pub async fn set_pushrule_route(
    ...
    if let Err(error) = account_data.content.global.insert(
        ...
        let err = match error {
            InsertPushRuleError::ServerDefaultRuleId => Error::BadRequestString(
                ErrorKind::InvalidParam,
                "Rule IDs starting with a dot are reserved for server-default rules"
            ),
            InsertPushRuleError::InvalidRuleId => Error::BadRequestString(
                ErrorKind::InvalidParam,
                "Rule ID containing invalid characters"
            ),
            InsertPushRuleError::RelativeToServerDefaultRule => Error::BadRequestString(
                ErrorKind::InvalidParam,
                "Can't place a push rule relatively to a server-default rule"
            ),
            InsertPushRuleError::UnknownRuleId => Error::BadRequestString(
                ErrorKind::NotFound,
                "The before or after rule could not be found"
            ),
            InsertPushRuleError::BeforeHigherThanAfter => Error::BadRequestString(
                ErrorKind::InvalidParam,
...
pub async fn delete_pushrule_route(
    ...
    {
        let err = match error {
            RemovePushRuleError::ServerDefault => Error::BadRequestString(
                ErrorKind::InvalidParam,
