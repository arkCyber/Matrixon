...
async fn migrate_to_sha256_media(
    ...
    let mxc_dimension_splitter_pos = media_info
        .iter()
        .position(|&b| b == 0xff)
        .ok_or_else(|| Error::bad_database("Invalid format of media info from file's name"))?;

    let mxc = utils::string_from_bytes(&media_info[..mxc_dimension_splitter_pos])
        .map(OwnedMxcUri::from)
        .map_err(|_| Error::bad_database("MXC from file's name is invalid UTF-8."))?;
    let (server_name, media_id) = mxc
        .parts()
        .map_err(|_| Error::bad_database("MXC from file's name is invalid."))?;
    ...
    let content_type = parts
        ...
        .map(|bytes| {
            utils::string_from_bytes(bytes)
                .map_err(|_| Error::bad_database("Content type from file's name is invalid UTF-8."))
        })
        .transpose()?;
