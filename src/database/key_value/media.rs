        ...
        let maybe_remove_remaining_metadata = |metadata: &DbFileMeta, errors: &mut Vec<Error>| {
                ...
                .scan_prefix(metadata.sha256_digest.clone())
            {
                if let Some(servername_mediaid) = k.get(32..) {
                    ...
                    errors.push(Error::bad_database(
            ...
            let thumbnail_id_error = || {
                ...
                Error::bad_database("Invalid format of value in filehash_thumbnailid")
            };
    ...
    fn list_blocked(&self) -> Vec<Result<BlockedMediaInfo>> {
        let parse_servername = |parts: &mut Split<_, _>| {
            OwnedServerName::try_from(
                utils::string_from_bytes(parts.next().ok_or_else(|| {
                    Error::bad_database("Invalid format of metadata of blocked media")
                })?)
                .map_err(|_| Error::bad_database("Invalid server_name String of blocked data"))?,
            )
            .map_err(|_| Error::bad_database("Invalid ServerName in blocked_servername_mediaid"))
        };
        ...
        let parse_string =
            |parts: &mut Split<_, _>| {
                ...
                .map_err(|_| Error::bad_database("Invalid string in blocked media metadata"))
            };
...
fn parse_metadata(value: &[u8]) -> Result<DbFileMeta> {
    let (sha256_digest, mut parts) = value
        ...
        .ok_or_else(|| Error::bad_database("Invalid format for media metadata"))?;

    let filename = parts
        .next()
        .map(|bytes| {
            utils::string_from_bytes(bytes)
                .map_err(|_| Error::bad_database("filename in media metadata is invalid unicode"))
        })
        .transpose()?
    ...
    let content_type = parts
        ...
        .map(|bytes| {
            utils::string_from_bytes(bytes).map_err(|_| {
                Error::bad_database("content type in media metadata is invalid unicode")
            })
        })
        .transpose()?
        .and_then(|s| (!s.is_empty()).then_some(s));
...
fn dimensions_from_thumbnailid(thumbnail_id: &[u8]) -> Result<(u32, u32)> {
    let (width, height) = thumbnail_id[thumbnail_id
        .len()
        .checked_sub(8)
        .ok_or_else(|| Error::bad_database("Invalid format of dimensions from thumbnailid"))?..]
        .split_at(4);
    ...
    fn get_u64_val(
        ...
        self.value
            ...
            .map_err(|_| {
                ...
                Error::bad_database(invalid_error)
            })
    }
